...
import me.earzuchan.chatdrama.client.viewmodel.DisplayTempFakeMessage
...
fun FakeMessageItem(message: DisplayTempFakeMessage) = Column(Modifier.fillMaxWidth(), horizontalAlignment = if (message.fromMe) Alignment.End else Alignment.Start) {
    if (message.time != null) Box(Modifier.padding(start = 16.dp,end = 16.dp,top = 8.dp,bottom = 4.dp)) {
    ...
    Box(Modifier.squircleBackground(if (message.fromMe) MiuixTheme.colorScheme.primary else MiuixTheme.colorScheme.background, 16.dp).padding(16.dp,12.dp)) {
        Text(message.content, color = if (message.fromMe) MiuixTheme.colorScheme.onPrimary else MiuixTheme.colorScheme.onBackground, style = MiuixTheme.textStyles.paragraph)
    }
}